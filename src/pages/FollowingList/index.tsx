...
import { FETCH_FOLLOWINGS_BY_USERNAME } from '../../graphql/operations/user';
import { IUser } from '../../types/user';

const FollowingList = () => {
  const { username } = useParams();

  const { loading, data, error } = useQuery(FETCH_FOLLOWINGS_BY_USERNAME, {
    variables: { username },
  });
    ...
    <>
      ...
      <section className="flex flex-col max-w-7xl mx-auto items-center space-y-5 p-5">
        ...
        <ul className="menu bg-base-100 w-full max-w-2xl rounded-box overflow-hidden">
          {data.fetchUserByUsername.followings.map((following: IUser) => (
            <div className="relative group" key={following.id}>
              <li
              ...
              <div className="absolute font-semibold flex space-x-5 group-hover:text-base-100 top-2.5 right-5 z-50">
                <p>
                  ...
                  {following.followerCount > 1 ? 'followers' : 'follower'}{' '}
                </p>
                <p>
                  {following.followingCount}{' '}
                  {following.followingCount > 1 ? 'followings' : 'following'}{' '}
                </p>
              </div>
            </div>
          ))}
